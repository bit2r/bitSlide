---
title: "chatGPT"
subtitle: "챗GPT 최신 동향"
author: "이광춘/신종화"
institute: "비영리법인 한국 R 사용자회"
date: 2023-05-18
format: 
  insper-revealjs:
    self-contained: false
    footer: |
      한국 R 사용자회 [chatGPT](https://r2bit.com/chatGPT/). theme [insper](https://github.com/padsInsper/quarto-insper-theme) 소스코드 [GitHub](https://github.com/padsInsper/quarto-insper-theme).
date-format: long
lang: ko
filters:
   - lightbox
lightbox: auto
bibliography: bibliography.bib
csl: apa-single-spaced.csl  
editor_options: 
  chunk_output_type: console
---

## 목차

1.  들어가며
1.  컴퓨터 기본지식과 역사
1.  체험 삶의 현장
1.  DT, 애자일, DevOps &rarr; 오피스 전쟁
1.  chatGPT란?
1.  chatGPT 사용자 관점
1.  chatGPT 앱 개발자 관점
1.  chatGPT 한계와 시사점
1.  구글 검색 위기 (챗GPT와 오정보)
1.  질의응답

{{< include slideshow/202305-gpt/00-who-am-i.qmd >}}

{{< include slideshow/202305-gpt/seoul-r-meeup.qmd >}}

{{< include slideshow/202305-gpt/00-open-source-software.qmd >}}

{{< include slideshow/202305-gpt/01-life-in-action.qmd >}}

{{< include slideshow/202305-gpt/01-dt-agile-devops.qmd >}}

{{< include slideshow/202305-gpt/02-about-chatGPT.qmd >}}

{{< include slideshow/202305-gpt/chatGPT-development.qmd >}}

{{< include slideshow/202305-gpt/03-chatGPT-in-practice.qmd >}}

{{< include slideshow/202305-gpt/04-limitation.qmd >}}

{{< include slideshow/202305-gpt/05-google-search.qmd >}}

{{< include slideshow/202305-gpt/reference.qmd >}}

